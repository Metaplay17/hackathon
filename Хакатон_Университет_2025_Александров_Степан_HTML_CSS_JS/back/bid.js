...
let predmetsDataBase = []; // Выбранные предметы по базы данных
...
function loadPredmets(){
    ...
    for(let i = 0 ; i < subjectsDataBase.length; i++){
        varEgeDataBase.push(subjectsDataBase[i]);
    }
    dop = varEge;
}
...
function save() {
    for (let i = 0; i < predmets.length; i++) {
        for (let j = 0; j < dop.length; j++) {
            if (predmets[i] == dop[j]) {
                predmetsDataBase.push(varEgeDataBase[i]);
            }
        }
    }
    const vib = {
